...
func TestTransportChecksRequestHeaderListSize(t *testing.T) {
	...
	cc, err := tr.GetConnPool().GetClientConn(req, addr)
	if err != nil {
...
func testTransportBodyReadError(t *testing.T, body []byte) {
	...
	ct.client = func() error {
		...
		checkNoStreams := func() error {
			cp, ok := ct.tr.GetConnPool().(*clientConnPool)
			if !ok {
				return fmt.Errorf("conn pool is %T; want *clientConnPool", ct.tr.GetConnPool())
			}
			cp.mu.Lock()
			defer cp.mu.Unlock()
